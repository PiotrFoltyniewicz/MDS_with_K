...
# get number of uncovered nodes in range k and also list of outer nodes in range
def get_outer_in_range(start_node, k):
    output = []
    queue = [(start_node, 0)]
    uncovered = 0
    visited = set()
...
def get_all_in_range(start_node, k):
    output = []
    queue = [(start_node, 0)]
    uncovered = 0
    visited = set()
...
def leaf_reduction(graph, k):
    ...
    for node in graph:
        ...
        # in case of k = 1, it works perfectly, i am not sure if it is the most optimal for k > 1
            ...
            possible_nodes, _ = get_outer_in_range(node, k)
            ...
            for knight in possible_nodes:
                _, cover_count = get_outer_in_range(knight, k)
...
def greedy_pick(graph, k):
    ...
    for node in graph:
        ...
        possible_nodes, node_cover = get_outer_in_range(node, k)
        best_node, highest_cover = node, node_cover

        for pos_node in possible_nodes:
            _, cover_count = get_outer_in_range(pos_node, k)
            if cover_count > highest_cover:
                best_node = pos_node
                highest_cover = cover_count
...
# just checks if all nodes in graph are marked as covered
# it is susceptible for mistakes
def shallow_validate(graph):
    return all([node.is_covered for node in graph])

# validates based on a solution
# slower, but necessary to test if algorithm gives valid solution
...
# automatic tester for test cases
def heuristic_tester(solver):
    ...
    for i in range(1, 11):
        ...
        print("test" + str(i).zfill(2) , deep_validate(graph, ans, k), len(ans), f"{elapsed_time:.2f}", sep='\t')

heuristic_tester(ramp_up_runner)
#optilio_run()